---
title: "Zwrot Fedu: rynek przestał wyceniać cięcia, zaczął wyceniać podwyżki"
description: "CPI 4.2%, blowout payrolls i jastrzębi Fed przestawiły rynek z cięć na podwyżki. Szanse na wrześniową podwyżkę skoczyły z 29% do 68%. Przełącznik, który tłumaczy mocny dolar, spadek złota i odpływy z krypto."
date: 2026-06-25 12:00:00 +0200
dek: "Przez pół roku rynek żył nadzieją na cięcia. Jeden lepki odczyt inflacji i jeden mocny raport z rynku pracy odwróciły narrację o 180 stopni. Rozkładam ten zwrot i jego skutki na wszystkich rynkach."
readingTime: 9
tags: [Fed, stopy procentowe, inflacja, dolar, makro, obligacje, rynki]
image: /assets/og.png
cover: macro
---

> **W skrócie**
>
> - Inflacja w USA wróciła do **4.2%**, a majowy raport z rynku pracy dorzucił **+172 tys.** etatów, ponad dwa razy więcej niż prognozy. To dwa gwoździe do trumny scenariusza cięć.
> - Rynek przestawił się gwałtownie: szanse na **wrześniową podwyżkę** skoczyły z **29% do 68%** w tydzień. Fed trzyma 3.50–3.75% jastrzębim tonem.
> - Krzywa to przyznaje. 2s10s spłaszczyła się z **~30 do 20 bp**: krótki koniec zakotwiczony przez Fed, długi zszedł z 4.50% do 4.41%, bo odprężenie wokół Iranu wypuściło premię za ryzyko.
> - To jeden przełącznik, a rusza wszystkim: dolar na rocznym maksimum, złoto pod 4000, odpływy z ETF-ów na bitcoinie. Mechanika jest wspólna.

**Teza:** rynek nie zmienił zdania o gospodarce, zmienił zdanie o Fedzie. Dopóki inflacja jest lepka, a praca mocna, każdy kolejny odczyt przesuwa wycenę w stronę wyższych stóp, nie niższych. To tło, w którym mocny dolar jest stanem domyślnym.

## Co pękło: od cięć do podwyżek

Przez pierwsze miesiące 2026 rynek żył jednym założeniem: Fed zacznie ciąć. To założenie pękło 5 czerwca. Majowy raport z rynku pracy pokazał +172 tys. etatów, ponad dwa razy powyżej prognoz. W połączeniu z inflacją, która wróciła do 4.2%, najwyżej od dwóch lat, zniknął argument za cięciem. Reakcja była natychmiastowa: złoto spadło tego dnia o 3.27%, bo rynek nagle zaczął bać się Fedu bardziej niż nagłówków.

Liczby z samego FOMC mówią to samo. Dziewięciu członków widzi w tym roku co najmniej jedną podwyżkę, sześciu co najmniej dwie. To nie jest komitet, który szykuje się do luzowania. Owszem, część analityków, jak J.P. Morgan, wciąż obstawia hold do końca roku, ale środek rozkładu wyraźnie przesunął się w górę.

<figure>
<svg viewBox="0 0 640 130" font-family="-apple-system,Segoe UI,Roboto,sans-serif" xmlns="http://www.w3.org/2000/svg"><defs><marker id="ar" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M2 1L8 5L2 9" fill="none" stroke="#0b66c3" stroke-width="1.5"/></marker></defs><text x="40" y="22" font-size="13" fill="currentColor">Szansa na wrześniową podwyżkę Fedu (wycena rynkowa)</text><rect x="40" y="40" width="560" height="16" rx="8" fill="#ececec"/><rect x="40" y="40" width="381" height="16" rx="8" fill="#0b66c3"/><line x1="202" y1="34" x2="202" y2="62" stroke="#9b9b9b" stroke-width="2" stroke-dasharray="3 3"/><text x="202" y="80" font-size="11" fill="#9b9b9b" text-anchor="middle">29% tydzień temu</text><line x1="421" y1="34" x2="421" y2="62" stroke="currentColor" stroke-width="2"/><text x="421" y="80" font-size="12" fill="#0b66c3" text-anchor="middle" font-family="monospace">68% teraz</text><path d="M208 104 L415 104" stroke="#0b66c3" stroke-width="2" marker-end="url(#ar)"/><text x="40" y="120" font-size="10" fill="#9b9b9b">0%</text><text x="600" y="120" font-size="10" fill="#9b9b9b" text-anchor="end">100%</text></svg>
<figcaption>Wycena rynkowa szans na wrześniową podwyżkę Fedu: skok z 29% do 68% w tydzień.</figcaption>
</figure>

## Krzywa to przyznaje

Najlepszym świadkiem nie są komentarze, tylko obligacje. Dwulatka, najczulsza na politykę Fedu, stoi przy 4.21% i nie chce zejść, bo rynek wycenia jastrzębia. Dziesięciolatka zeszła z 4.50% do 4.41%, ale z innego powodu: odprężenie w relacjach USA i Iranu wypuściło premię za ryzyko geopolityczne, ropa staniała, długi koniec krzywej się rozluźnił.

Efekt to spłaszczenie. Spread 2s10s skurczył się z około 30 do 20 punktów bazowych. Krzywa wciąż nie jest odwrócona, więc to nie jest jeszcze sygnał recesyjny, ale kierunek jest jednoznaczny: krótki koniec trzymany przez Fed, długi schodzący na geopolityce. Realna rentowność dziesięciolatki to +0.21%, dodatnia, co dalej pracuje na korzyść dolara.

<figure>
<svg viewBox="0 0 640 172" font-family="-apple-system,Segoe UI,Roboto,sans-serif" xmlns="http://www.w3.org/2000/svg"><text x="40" y="22" font-size="13" fill="currentColor">Krzywa się spłaszcza: krótki koniec zakotwiczony</text><line x1="120" y1="137" x2="540" y2="137" stroke="#e6e6e6"/><line x1="180" y1="100" x2="470" y2="42" stroke="#9b9b9b" stroke-width="1.5" stroke-dasharray="4 4"/><text x="480" y="42" font-size="10" fill="#9b9b9b">było 4.50</text><line x1="180" y1="100" x2="470" y2="60" stroke="#0b66c3" stroke-width="2.5"/><circle cx="180" cy="100" r="5" fill="#0b66c3"/><circle cx="470" cy="60" r="5" fill="#0b66c3"/><text x="180" y="88" font-size="12" fill="currentColor" text-anchor="middle" font-family="monospace">2Y 4.21</text><text x="470" y="48" font-size="12" fill="currentColor" text-anchor="middle" font-family="monospace">10Y 4.41</text><text x="320" y="160" font-size="12" fill="#6b6b6b" text-anchor="middle">spread 2s10s: +20 bp (spłaszczenie z ~30 bp)</text></svg>
<figcaption>Spłaszczenie krzywej: dwulatka zakotwiczona przez Fed, dziesięciolatka zeszła na odprężeniu wokół Iranu.</figcaption>
</figure>

## Jeden przełącznik, wszystkie aktywa

Tu robi się ciekawie dla każdego, kto handluje czymkolwiek. Ten sam zwrot Fedu tłumaczy ruchy na rynkach, które pozornie nie mają ze sobą wiele wspólnego.

Dolar jest na rocznym maksimum, DXY powyżej 100, bo wyższe realne stopy to magnes na kapitał. Złoto spadło pod 4000 pierwszy raz od listopada, kasując cały zysk 2026, bo kruszec bez odsetek przegrywa z dodatnią realną stopą i mocnym dolarem. Bitcoin obrywa z tej samej strony: fundusze wyciągnęły z ETF-ów rekordowe kwoty, bo droższy pieniądz i mocny dolar odbierają paliwo aktywom ryzykownym. Nawet ropa, choć tu głównym powodem jest Iran, dokłada się do układanki, bo tańsza energia zdejmuje część presji inflacyjnej z dłuższego końca.

<figure>
<svg viewBox="0 0 640 184" font-family="-apple-system,Segoe UI,Roboto,sans-serif" xmlns="http://www.w3.org/2000/svg"><line x1="300" y1="10" x2="300" y2="172" stroke="#e6e6e6"/><rect x="300" y="16" width="120" height="18" rx="3" fill="#1a9e6a"/><text x="40" y="30" font-size="12" fill="currentColor">Dolar (DXY)</text><text x="632" y="30" font-size="10.5" fill="#6b6b6b" text-anchor="end">rok-max, > 100</text><rect x="120" y="56" width="180" height="18" rx="3" fill="#d64545"/><text x="40" y="70" font-size="12" fill="currentColor">Złoto</text><text x="632" y="70" font-size="10.5" fill="#6b6b6b" text-anchor="end">−29% od szczytu 5602</text><rect x="150" y="96" width="150" height="18" rx="3" fill="#d64545"/><text x="40" y="110" font-size="12" fill="currentColor">Bitcoin</text><text x="632" y="110" font-size="10.5" fill="#6b6b6b" text-anchor="end">rekordowe odpływy z ETF</text><rect x="140" y="136" width="160" height="18" rx="3" fill="#d64545"/><text x="40" y="150" font-size="12" fill="currentColor">Ropa (Brent)</text><text x="632" y="150" font-size="10.5" fill="#6b6b6b" text-anchor="end">−36% od szczytu</text></svg>
<figcaption>Jeden zwrot Fedu, reakcja w wielu klasach aktywów. Zielony wzmacnia, czerwony osłabia.</figcaption>
</figure>

| Wskaźnik | Wartość | Implikacja |
|---|---|---|
| CPI USA | 4.2% | lepka, ogranicza Fed |
| Payrolls (maj) | +172 tys. | ponad 2x prognoza |
| Szansa podwyżki IX | 29% → 68% | repricing w tydzień |
| Fed | 3.50–3.75% | hold jastrzębi |
| 2Y / 10Y | 4.21% / 4.41% | krótki kotwiczony |
| 2s10s | +20 bp | spłaszczenie z ~30 |
| Realna 10Y | +0.21% | magnes na kapitał |
| DXY | > 100 | rok-maksimum |

## Co musiałoby to odwrócić

Teza wisi na dwóch liczbach: inflacji i rynku pracy. Słabszy raport z payrolls, wyraźny zjazd CPI albo nagły szok wzrostowy zmieniłyby kalkulację Fedu i otworzyły miejsce na powrót narracji o cięciach. Dopóki te dwie liczby trzymają, scenariusz bazowy to wyższe stopy na dłużej, nie luzowanie. To nie prognoza konkretnego ruchu, tylko mapa tego, co rynek aktualnie wycenia i co musiałoby pęknąć.

## Jak to czytać

To kontekst i mechanika, nie rekomendacja. Pokazuję, dlaczego jeden zwrot w polityce Fedu rządzi dziś tak wieloma rynkami naraz i jakie liczby trzymać na oku. Decyzję podejmujesz sam. To nie porada inwestycyjna.

Źródła: [EBC: złoto pod 4000 a Fed](https://www.ebc.com/forex/gold-price-drop-xauusd-below-4000-fed-june-2026) · [J.P. Morgan: następny ruch Fedu](https://www.jpmorgan.com/insights/global-research/economy/fed-rate-cuts) · [iShares: Fed Outlook 2026](https://www.ishares.com/us/insights/fed-outlook-2026-interest-rate-forecast)

OBSERVE_ONLY · MANUAL_DECISION_ONLY · NO_AUTO_TRADING
